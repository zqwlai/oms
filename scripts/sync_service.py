...
def add(cluster, service, host, port, admin_user='', admin_password=''):
    if Service.objects.filter(fcluster=cluster, fhost=host, fport=port ):
        if admin_user or admin_password:
            Service.objects.filter(fcluster=cluster, fhost=host, fport=port ).update(fadmin_user=admin_user, fadmin_password=admin_password)
    else:
...
def sync_service_from_mc(host, port, user, password, db):
    s = Mysql(host, int(port), user, password, db)
...
if TConfig.objects.filter(fkey='mc'):
    config = TConfig.objects.get(fkey='mc').fvalue
else:
    sys.exit(0)

for each_config in config.strip().split():
    temp = each_config.split('|')
    sync_service_from_mc(*temp)
